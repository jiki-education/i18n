...
La función `ellipse` (elipse) recibe 5 entradas: la posición central (`centerX` (coordenada X del centro), `centerY` (coordenada Y del centro)), el radio horizontal (`radiusX` (radio horizontal)), el radio vertical (`radiusY` (radio vertical)) y el color:
...
Hemos dibujado la mitad de la imagen por ti. Tu tarea es dibujar el lado derecho del pingüino. El dibujo final debería verse así:
...
A diferencia de los ejercicios anteriores, el objetivo es resolverlo **SIN** pasar el cursor sobre las coordenadas ni adivinarlas.

Al leer el código que está allí, deberías poder deducir los demás detalles para que quede simétrico. Recuerda que la esquina superior izquierda del lienzo es `0,0` y la esquina inferior derecha es `100,100`. El pingüino está sentado en el centro.

Para el pico, debes **cambiar** las coordenadas centrales del triángulo. No añadas un triángulo nuevo.
