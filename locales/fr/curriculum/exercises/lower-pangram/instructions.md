...
Ta tâche est d'écrire `isPangram(sentence)` (« est un pangramme »), qui prend une phrase en entrée et renvoie `true` si elle contient chaque lettre de « a » à « z » au moins une fois, ou `false` dans le cas contraire.

### Fonctions auxiliaires

Pour réussir cet exercice, tu n'as pas le droit d'imbriquer une boucle dans une autre. À la place, comme dans l'exercice précédent, la clé est de créer une **fonction auxiliaire** que `isPangram(...)` utilise.

Cette fonction auxiliaire t'aidera à déterminer si une string en contient une autre. On te recommande de l'appeler `includes` et de nommer ses entrées `haystack` (la botte de foin) et `needle` (l'aiguille). L'idée est que l'on cherche à « trouver l'aiguille dans la botte de foin », comme le dit l'expression bien connue.
