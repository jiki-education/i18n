...
Pour définir la couleur, utilise la fonction `hsl(...)` (teinte, saturation, luminosité). Elle renvoie (_returns_ en anglais) une _string_ de couleur que tu peux ensuite utiliser comme dernier argument de `rectangle(...)`. Le mieux est sans doute de régler la saturation et la luminosité (_lightness_ en anglais) autour de `50`. La première barre doit avoir une **teinte** (_hue_ en anglais) de `0`. La teinte doit augmenter à chaque barre, pour finir aux alentours de 300.
