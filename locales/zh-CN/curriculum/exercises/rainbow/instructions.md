...
要设置颜色，请使用 `hsl(...)`（色相、饱和度、亮度函数）。`hsl(...)` 会返回（英文：_returns_）一个颜色字符串，然后你可以在 `rectangle(...)`（绘制矩形的函数）中将其作为最后一个输入参数。将饱和度（英文：_saturation_）和亮度（英文：_lightness_）的值设置成大约 `50` 可能最合适。第一条的**色相（英文：*hue*）**应为 `0`。每条竖条的色相应逐渐增加，最后结束时达到 300 左右。
