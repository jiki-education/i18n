---
title: "循环中的循环"
description: "将一个循环放入另一个循环里，这样内部循环会在外部循环的每一步完整运行一次，非常适合绘制网格。"
...
关键要理解的是，Jiki 会先完成内层循环（英文：inner loop）中的所有工作，然后再回到外层循环（英文：outer loop）。他并不会来回跳跃。他完成内层循环，然后才继续外层循环的下一步。
