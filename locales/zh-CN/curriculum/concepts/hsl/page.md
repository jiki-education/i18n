---
title: "HSL 色彩"
description: "从色相（色调，0–360）、饱和度（鲜艳程度，0–100）和明度（明亮程度，0–100）构建颜色。"
en_md5: d751e07ccf6b5a58a11d3d2af1026b93
---

HSL 给了我们一种与 RGB 完全不同的思考颜色的方式。HSL 代表 色相（英文：hue (H)）、饱和度（英文：saturation (S)）和 明度（英文：lightness (L)）。你有一个叫 `hsl` 的函数，它接受三个数字——一个代表色相，一个代表饱和度，一个代表明度——它会返回一个颜色。那么，我们来分解一下这三个词的意思。
