---
title: "场景"
description: "练习会以不同的初始条件来运行你同一份代码，因此你需要让代码对每种条件都适用。"
...
以高尔夫游戏为例。你可能需要根据 `getShotLength`（获取击球距离）函数的返回值（英文：return value）来调整球滚动的距离，该函数会告诉你球手击球有多远。想象一下，玩家在手机上滑动来输入击球长度。
