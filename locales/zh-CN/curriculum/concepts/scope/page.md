---
title: "作用域"
description: "理解花括号如何创建它们自己的一组货架，让变量只在其代码块运行期间存在。"
...
嗯，事实证明代码没问题。这是完全合法的 JavaScript，它之所以能工作，是因为一个叫做作用域（英文：_scope_）的概念。
...
所以当你创建一个变量时，你需要决定它要存在多久。如果你想让它长期存在，就需要把它放在所有花括号外面。如果你想让它只持续一次迭代（英文：_iteration_），也就是循环的一次执行期间，那么你可以把它放在花括号里。你能看出来这有多容易让人困惑吗？所以，在你刚开始学习时，有一个让事情变简单的好方法：把所有变量放在代码顶部、花括号外面，然后只在花括号里更新它们。比如，我们可以把上面的代码改成只用一个 `left` 和一个 `top` 盒子，初始值设为零，然后在循环里每次更新它们。这也完全没问题。这两种方式没有对错之分。有一些最佳实践，但我们以后会学到。现在嘛，尽情尝试，玩得开心，看看哪种方式让你感觉最舒服，不用太纠结这些。
