---
title: "After Party"
description: "Consulta quantos acompanhantes uma celebridade pode trazer, usando dois arrays alinhados entre si."
...
Hoje à noite, tens duas listas na tua prancheta. A primeira é `names`, os nomes completos de todos os convidados. A segunda é `plusOnes`, quantos acompanhantes cada um desses convidados pode trazer consigo (os seus "acompanhantes" (_plus-ones_ em inglês)). As duas listas foram escritas em conjunto, entrada por entrada e pela mesma ordem, para ficarem alinhadas entre si. Por exemplo, a terceira entrada do array `names` corresponde à mesma pessoa que a terceira entrada do array `plusOnes`.

Escreve uma função chamada `plusOnesFor` (o número de acompanhantes de uma pessoa). A função tem três parâmetros de entrada: o array `names`, o array `plusOnes` e o **primeiro nome** de quem está à tua frente neste momento. Deves devolver o número de acompanhantes que essa pessoa pode trazer.
